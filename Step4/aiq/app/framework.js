/*! aiq-app-framework - v1.1.1- - 2014-06-11 */
...
aiq.app = Spine.Class.extend({
    ...
    hashChange: function (args) {
        ...
        if (wasRenderFunctionCompleted !== undefined && previousRoute !== this.route) {
            // To avoid the bug when scroll position is kept when navigating to the next view
            $('body').scrollTop(0);
...
aiq.app.Controller.extend({
    ...
    registerAs: function (route, templateName) {
        ...
        if (templateName) {
            var path = 'templates/' + templateName;
            $.ajax({
                url: path,
                ...
                error: function (jq, status, message) {
                    console.error('Request of \'' + this.url + '\' failed. ' + message);
                }
            });
        }
    ...
    bindServerEvents: function() {
        ...
        var bind = function() {
            aiq.datasync.bind("document-created", {
                _type: that.className,
                callback: that.proxy(that.documentCreated)
            });
            aiq.datasync.bind("document-updated", {
                _type: that.className,
                callback: that.proxy(that.documentUpdated)
            });
            aiq.datasync.bind("document-deleted", {
                _type: that.className,
                callback: that.proxy(that.documentDeleted)
            });
        };
        if (aiq.ready) {
            bind();
        } else {
            document.addEventListener('aiq-ready', bind, false);
        }
    },
...
aiq.app.storage.Model.include({
    ...
    create: function(options) {
        ...
        // local storage allows forcing document identifiers so they
        // have to be copied from spine 'id' into core '_id' before
        // passing them to the core layer.
        ...
        // this.constructor.className because className is a class
        // property, so it must be accessed from the class context.
        aiq.storage.createDocument(this.constructor.className, attributes, {
            success: function(document) {
                that.id = document._id;
                that.cid = document._id;
                var record = that.dup(false);
